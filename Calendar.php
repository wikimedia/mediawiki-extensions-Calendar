...
class CalendarTable {
	...
	function getMonthName($aMonth, $aLen) {
		...
		if ($this->lng == 'user') $out = wfMsg( $mMonthMsgs[$aMonth - 1] );
		else $out = wfMsgForContent( $mMonthMsgs[$aMonth - 1] );
		if ($aLen > 0) $out = substr( $out, 0, $aLen );
		return ucfirst( $out);
	}
	...
	function getWeekday($aDay, $aLen) {
		...
		if ($this->lng == 'user') $out = wfMsg( $mWeekdayAbbrevMsgs[$aDay % 7] );
		else $out = wfMsgForContent( $mWeekdayAbbrevMsgs[$aDay % 7] );
		if ($aLen > 0) $out = substr( $out, 0, $aLen );
		return ucfirst( $out);
	}
	...
	function buildTable () {
		...
		if (($this->weekStart > 0) and ($day1 == 0)) $day1 = 7;
		$days = $this->daysInMonth ($this->month, $this->year);

		$style = '';
		$result = 'calendar';
		switch ($this->position) {
			case 'left': $result .= ' calLeft'; break;
			case 'right': $result .= ' calRight'; break;
			case 'center': $result .= ' calCenter'; $style = ' align="center"';
		}

		if ($this->tableWidth == 'default') $result .= ' calWidth';
		else if ($this->tableWidth != 'none') $style .= ' style="width: ' . $this->tableWidth . '"';
		...
		if ($this->nextLink == '') $colSpan++;
		if ($this->prevLink == '') $colSpan++;
		else $result .= '| class="prevNext" | ' . $this->prevLink . "\n";
		$result .= '| class="calTitle" colspan="' . $colSpan . '" | ' . $this->calTitle . "\n";
		if ($this->nextLink != '') $result .= '| class="prevNext" | ' . $this->nextLink . "\n";

		$result .= "|-\n";
		for ($i = $this->weekStart; $i < 7; $i++) $result .= "! " . $this->getWeekdayAbbrev($i, $this->dayCharsCount) . "\n";
		if ($this->weekStart > 0) for ($i = 0; $i < $this->weekStart; $i++)
		...
		while ($c <= $days) {
			...
			for ($i = 0; $i < 7; $i++) {
				...
				$styles=array();
				if (($i == 0) and ($this->weekStart == 0) ) $styles[] = 'sundays';
				if (($i == 6) and ($this->weekStart == 1) ) $styles[] = 'sundays';
				if ($this->showToday and ($c == $this->curDay) and ($this->curMonth == $this->month)
					and ($this->curYear == $this->year)) $styles[] = 'today';
				if (in_array($c, $this->highlightedDays)) $styles[] = 'highlighted';
				$allStyles = implode (' ', $styles);
				if ($allStyles == '') $result .= '| ';
				else $result .= '| class="' . $allStyles . '" | ';
				if (($c > 0) and ($c <= $days)) {
					if ($this->dailyLinks[$c] != '')
						$result .= '[[' . $this->dailyLinks[$c] . '|' . $c . ']]';
					else if ($this->generalLinks != '')
	...
	function setParameters ( &$args ) {
		...
		if (count($args) > 0) {
			foreach($args as $arg) {
				...
				if ((count($parts) == 2) and ($parts[0] > 0) and ($parts[0] < 32))
				...
				else if (count($parts) == 2) switch ($parts[0]) {
					case 'month':
					...
					case 'lang':
						...
						if ($this->lng != 'user') $this->lng='content';
						break;
					case 'prevLink':
					...
					case 'start':
						...
						if ($this->weekStart != 1) $this->weekStart = 0;
						break;
					case 'position':
					...
					case 'dayCharsCount':
						...
						if ($this->dayCharsCount < 0) $this->dayCharsCount = 0;
						break;
					case 'monthCharsCount':
						$this->monthCharsCount = intval ($parts[1]);
						if ($this->monthCharsCount < 0) $this->monthCharsCount = 0;
						break;
					case 'tableWidth':
					...
					case 'showToday':
						...
						if ($this->showToday == 'true') $this->showToday = true;
						else if ($this->showToday == 'false') $this->showToday = false;
		...
		if ($nextMonth == 13) { $nextMonth = 1; $nextMonthYear++; }
		$this->monthArr = array (
